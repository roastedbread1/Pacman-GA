...
class Game:
    def __init__(self):
        ...
        #
        self.isInputInjected = False
        self.inputs = []
        self.readInputFrame = 1
        self.frameElapsed = 0
        #
    ...
    #
    def injectInput(self, inputs, readInputFrame):
        self.isInputInjected = True
        self.inputs = inputs
        self.readInputFrame = readInputFrame
    #
    ...
    def run(self):
        ...
        while self.running:
            ...
            if (self.isInputInjected and self.frameElapsed % self.readInputFrame == 0):
                dir = self.inputs.next()
                self.pacman.readMovement(dir)             
            self.frameElapsed += 1
            #

            for event in pygame.event.get():
                if (not self.isInputInjected):
